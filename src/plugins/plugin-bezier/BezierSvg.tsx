...
import type { BezierProps, Bezier as BezierType } from "./bezier-types";
import { useInvertedRange, useRange } from "./bezier-utils";
...
export function BezierSvg({
  ...
  const bind = useDrag(({ xy: [x, y], event, first, memo }) => {
    if (first) {
      ...
      memo = [handleLeft.current, handleRight.current].indexOf(
        event!.target as any
      );
