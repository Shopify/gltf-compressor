...
const optionValues = [
  false,
  ...Object.values(BuiltIn).map((c) => c.toString()),
];
...
function SelectBezier({
  value,
  onUpdate,
}: Pick<BezierProps, "value" | "onUpdate">) {
  const selectValue = useMemo(
    () => optionValues.find((v) => v === value.toString()) || false,
    [value]
  );
  const args = { type: "SELECT", value: selectValue, settings: selectSettings };
  const setValue = (newValue: string | boolean) =>
    newValue && onUpdate((newValue as string).split(","));
...
export function Bezier() {
  const { label, value, displayValue, settings, onUpdate, setSettings } =
    useInputContext<BezierProps>();
    ...
    <>
      <Row input>
        ...
        <Container>
          <SyledInnerLabel
            graph={graph}
            onClick={() => setSettings({ graph: !graph })}
          >
            𝑓
          </SyledInnerLabel>
